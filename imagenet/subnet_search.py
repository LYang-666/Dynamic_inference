...
def main():
    ...
    # split validate data
    length = [2500, len(train_data)-2500]
    vali_data, tes_data = random_split(train_data, length)
    ...
    arch_iter = []
    index_list = torch.zeros(19)
    index_list_inverse = torch.zeros(19)

    m_channels = torch.zeros(19)
    ll = 0
    l_skip = torch.zeros(19)
    for m in net.modules():
        if isinstance(m, nn.Conv2d):
            if m.kernel_size[0] is not 1:
    ...
    for num in range(num_iter):
        ...
        for m in net.modules():
            if isinstance(m, nn.Conv2d):
                if m.kernel_size[0] is not 1:
                    ...
                    current_idx = int(index_list[l])
                    #update the num_channels in current layer
        
                    # number of channels reduces 1
                    net.ch_width[l] -= 1
                    # the pruned channel index
                    net.ch_index[l].append(int(index[current_idx]))
                    net.update_model(l)
                    acc, loss = validate(vali_loader, net, criterion, log)
                    print('Layer: {},  Channel: {},   Acc: {}'.format(l, int(index[current_idx]), acc))

                    # recover 
                    net.ch_width[l] += 1
                    net.ch_index[l].remove(int(index[current_idx])) 
                    net.update_model(l)
        ...
        channel_layer_acc_list.append([layer, channel, acc, M_flops, M_params])

        #l_c_list array list all pruned channles index in current layers
        l_c_list[layer].append(channel)
        # for l in range(19):
        #     channel_in_list = []
        #     for c in range(int(len(channel_layer_acc_list))):
        #         if int(channel_layer_acc_list[c][0]) == l:
                    
        #             channel_in_list.append(channel_layer_acc_list[c][1])
        #     l_c_list[l] = channel_in_list
            # l_c_list.append(channel_in_list)
        ...
        print_log(
            ...
            format(time_string(), num, num_iter, need_time), log)
        print_log(" [Current iteration:{}] | [The layer:{}] | [The channel:{}] | [Current accuracy:{:2f}] | [Current Flops and params:{},{}".
            format(num, layer, channel, acc1, M_flops, M_params), log)

        iter_time.update(time.time() - start_time)
        start_time = time.time()

        sparse = countZeroWeights(net)
        zero_ch = countZeroChannles(net)
        print_log("Current sparsity:{}".format(sparse), log)
        print_log("Current zero channels:{}".format(zero_ch), log)

        np.save(args.save_path+'/channel_layer_acc_list', channel_layer_acc_list)
        np.save(args.save_path+'/arch_iter', arch_iter)    
        if M_flops < max_flop/16:
            break     
...
def fine_tuning(train_loader, vali_loader, net,  l_c_list, criterion,optimizer, writer, recorder, log, i):
        ...
        for epoch in range(args.epochs):
            ...
            print('test_accuracy:{}'.format(val_acc))
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for m in model.modules():
            if isinstance(m, nn.Conv2d):
                ...
                # m.alpha_w.grad = m.alpha_w.grad * 0

                if m.kernel_size[0] is not 1:
                    channel_width = layer_list[l]
                    if len(channel_width) is not 0:
                    ...
                    if len(layer_list[l-1]) is not 0:
                        channel_width = layer_list[l-1]
                        m.weight.data[channel_width] = 0         
                        m.weight.grad.data[channel_width] = 0
                        m.alpha_w.data[channel_width] = 0
                        m.alpha_w.grad.data[channel_width] = 0
