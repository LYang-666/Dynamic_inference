...
def main():
    ...
    # split validate data
    length = [10000, len(train_data)-10000]
    ...
    # pretrained_dict = torch.load('save/mobilenetv2_1.0-0c6065bc.pth')
    ...
    _Num_Layer = ll
    print('Num of layers:{}'.format(_Num_Layer))
    ...
    arch_iter = []
    index_list = torch.zeros(_Num_Layer)
    index_list_inverse = torch.zeros(_Num_Layer)

    m_channels = torch.zeros(_Num_Layer)
    ll = 0
    l_skip = torch.zeros(_Num_Layer)
    for name, m in net.named_modules():
        if isinstance(m, nn.Conv2d):
            if 'shortcut' not in name and '.conv2' not in name:
    ...
    for num in range(_Num_iter):
        ...
        for name, m in net.named_modules():
            if isinstance(m, nn.Conv2d):
                if 'shortcut' not in name and '.conv2' not in name:
                    ...
                    print('Layer: {},  Num_Channel: {},  Acc: {}'.format(l, m.out_channels, acc))
                    ...
                    net.module.ch_index[l] = [i for i in net.ch_index[l] if i not in index[current_idx].tolist()]
        ...
        if index_list[layer] == m_channels[layer]-int(m_channels[layer]/group)*2:
            l_skip[layer] = 1
        ...
        net.module.ch_index[layer] = channel
