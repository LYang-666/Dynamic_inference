...
def main():
    ...
    length1 = [int(len(train_data)/4), len(test_data)-int(len(train_data)/4)]
    vali_data, tes_data = random_split(test_data, length)
    train_data1, _ =  random_split(test_data, length)
    ...
    for num in range(_Num_iter):
        ...
        for name, m in net.named_modules():
            if isinstance(m, nn.Conv2d):
                if '.conv1' not in name[7:]:
                    ...
                    alpha, index = m.alpha_w.data.squeeze().sort(descending=True)
        ...
        if index_list[layer] == int(m_channels[layer] * 0.75) - int(m_channels[layer]/group):
